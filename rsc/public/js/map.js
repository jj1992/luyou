...
//定义地图图层的ID		
	...
	//添加地图加载时点击地图的监听事件
			...
			function rangeSearchs(pt,scope){ 
				// console.log(pt,scope)
    			map.graphics.clear();
    			...
    			var g = new Graphic(geom,sym);
    			g.setAttributes({"SEL_LYNAME":"buffer_circle"});
    			map.graphics.add(g);
    			var lyids = [0,3];
               ...
               _identifyTask.execute(identifyParams,lang.hitch(this,function(results){
				//    console.log(results)
            	   if(results.length > 0){
					...
					//    console.log(retinfos)
					   gisParams.JZWandFZDgather(retinfos);
            	   }else{
					gisParams.showError("未查询到资源");
            	   }
               }),function(err){     
				gisParams.showError("查询数据错误");       	   
               });        	  
			}
	...
	//添加管段图层gd layer
			function gdControls(gdControl){
				...
				gdLayer.setSelectionSymbol(getMarkerSybmol("aa"));
	...
	//添加杆路段图层gld layer
			function gldControls(gldControl){
				...
				gldLayer.setSelectionSymbol(getMarkerSybmol("aa"));
...
//----------------------------------------------------------------------------------------------------------
	...
	function ziYuanLocation(ids,lyid,guanglanId){
	    if(!ids){    
	      return;
	    }
...
        for(var i=0;i<results.features.length;i++){
		if(lyid == 5 || lyid == 7){
			...
			gySearchGraphic.setSymbol(getMarkerSybmol("aa"));
			...
			function clickFeatureEvent(layer,evt){
				...
				if("default" == mapOprate.curTool){
					...
					if(LayerIDConstant.FZD == lyid){
						...
						gdSelGraphic.setSymbol(getMarkerSybmol("aa"));
						gdSelGraphic.draw();
					}
						...
						gldSelGraphic.setSymbol(getMarkerSybmol("aa"));
						gldSelGraphic.draw();
					}
